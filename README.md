...
##Reliability
This approach correctly infers the 3D structures of nearly all structured internal loops when using as input sequences from 3D structures, a first validation step.  In most cases, a single sequence is enough to correctly infer the 3D structure. The probabilistic models for 3D motifs extracted from structurally conserved regions of 5S, 16S, or 23S rRNA were validated by scoring sequence variants from multiple sequence alignments that are different from those used to construct the models. While the NR dataset already includes a wide range of recurrent motifs, we expect new motifs as new structures are solved. We therefore have created a pipeline to automatically identify new motifs in new structures as they are released by PDB and to create new SCFG/MRF models. Using JAR3D webserver users can input motif sequences and obtain the most likely 3D structures, or a statistical indication that the motif is likely to be new.
