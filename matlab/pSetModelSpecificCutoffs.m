...
if ~exist('RandomData'),
	load([OutputPath filesep loopType '_Random_Sequence_Data_FP_Only.mat']);
	SequenceData = FPData;
	clear FPData;
	RandomData = SequenceData;
	clear SequenceData

	if isfield(RandomData,'Percentile'),
		RandomData = rmfield(RandomData,'Percentile');
	end

	RandomDataMotifIDs = cell(1,length(RandomData));
	for i = 1:length(RandomData),
		RandomDataMotifIDs{i} = RandomData(i).MotifID;
	end

	fprintf('Loaded data from %d random sequences\n',length(RandomData));
end
...
for iii = 1:length(GroupData),
	...
	% ------------------------ filter out poor random sequence matches

	k = find(ismember(RandomDataMotifIDs,CurrentMotif));
  SequenceData = RandomData(k);

	def = cat(1,SequenceData.Deficit);
	k = find(def <= Params.DeficitCutoff);
	SequenceData = SequenceData(k);
  fprintf('Removed sequences based on alignment score deficit, %d sequences remaining\n',length(SequenceData));

	[y,k] = sort(rand(1,length(SequenceData)));                            % randomize order
	SequenceData = SequenceData(k);
	AllSD = SequenceData;
	RSNum = length(SequenceData);
	fprintf('%10d sequences generated randomly deficits between %d %d\n', RSNum, 0, Params.DeficitCutoff);
  ...
  if length(SequenceData) > 0,
	...
	for q = 1:GroupData(motifnum).NumInstances,
		NSD(co).Deficit      = max(GroupData(motifnum).OwnScore) - GroupData(motifnum).OwnScore(q);
		NSD(co).CoreEdit     = 0;
		NSD(co).FullEdit     = 0;
		NSD(co).Conserved    = GroupData(motifnum).NumNT;
		NSD(co).NumInstances = GroupData(motifnum).NumInstances;
		NSD(co).MLPS         = GroupData(motifnum).OwnScore(q);
		NSD(co).Length       = length(GroupData(motifnum).OwnSequence{q}) - length(strfind(GroupData(motifnum).OwnSequence{q},'*'));
	...
	SequenceData = [NSD SequenceData];                  % put sequences from 3D first

	NSr = min(2*length(SequenceData),length(AllSD));                       % number of random sequences
  NSa = min(length(SequenceData),2*length(AllSD));                       % number of alignment sequences

	NSr = length(AllSD);                              % number of random sequences
  NSa = length(SequenceData);                       % number of alignment sequences

	AllSD = [AllSD(1:NSr) SequenceData(1:NSa)];            % use all sequences from both sources

	fprintf('Using %d random sequences and %d from the alignment and/or 3D structures\n', NSr, NSa);
